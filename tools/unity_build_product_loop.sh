...
FOURFOLD_BUILD_DIR="${FOURFOLD_BUILD_DIR:-Build/FourfoldEchoes}"
RUN_AFTER_BUILD=0

usage() {
  cat <<'USAGE'
Usage: tools/unity_build_product_loop.sh [--target macos|windows] [--output-dir PATH] [--run]

Builds the current product loop from the terminal:
...
Options:
  ...
  --output-dir PATH        Build output root. Defaults to ignored Build/FourfoldEchoes.
...
case "$FOURFOLD_BUILD_TARGET" in
  macos|osx|standaloneosx)
    ARTIFACT_PATH="$FOURFOLD_BUILD_DIR/macos/FourfoldEchoes.app"
    ;;
  windows|win64|standalonewindows64)
    ARTIFACT_PATH="$FOURFOLD_BUILD_DIR/windows/FourfoldEchoes.exe"
    ;;
  *)
...
"$UNITY_EDITOR" \
  ...
  -executeMethod FourfoldEchoes.Editor.FourfoldUnityBuild.BuildCurrentProductLoop \
  -logFile - \
  --fourfoldBuildTarget "$FOURFOLD_BUILD_TARGET" \
  --fourfoldBuildDir "$FOURFOLD_BUILD_DIR"

if [[ ! -e "$ARTIFACT_PATH" ]]; then
  echo "Product loop build artifact missing after successful Unity command: $ARTIFACT_PATH" >&2
  exit 1
fi

ARTIFACT_SIZE="$(du -sh "$ARTIFACT_PATH" | awk '{print $1}')"
echo "Product loop build artifact: $ARTIFACT_PATH"
echo "Product loop build artifact size: $ARTIFACT_SIZE"
