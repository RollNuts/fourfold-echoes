...
| `Assets/Scripts/FourfoldProofAudio.cs` runtime tones and ambient bed | Repository-authored procedural synthesis | Approved | Attack, hit, dodge, altar heat, gate open, room clear, enemy tell, player hit, phase, reward cues, and the low Gate A ambient loop are generated in Unity at runtime from sine-wave sample data. No external audio files or AI-generated audio assets are committed. |
