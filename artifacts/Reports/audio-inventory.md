# Audio Inventory

Generated UTC: `2026-06-25T20:05:03.451Z`

Imported AudioClip count from Unity validation: `10`

## Registered Cues

| Asset | Category | Status | License | Unity Path | Acceptance |
| --- | --- | --- | --- | --- | --- |
| `audio.sfx.normal_swing` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/attack_basic.wav | d020_pilot |
| `audio.sfx.enemy_hit_confirm` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/hit_enemy.wav | d020_pilot |
| `audio.sfx.enemy_death` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/enemy_death.wav | d020_pilot |
| `audio.sfx.player_dodge` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/dodge.wav | d020_pilot |
| `audio.sfx.relic_pickup` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/relic_pickup.wav | d020_pilot |
| `audio.sfx.tool_pulse` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/tool_pulse.wav | d020_pilot |
| `audio.sfx.tool_target_hit` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/tool_target_hit.wav | d020_pilot |
| `audio.sfx.tool_fail` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/tool_fail.wav | d020_pilot |
| `audio.sfx.shortcut_open` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/shortcut_open.wav | d020_pilot |
| `audio.sfx.discovery_stinger` | SFX | generated_pilot | repository-authored | Assets/Audio/Generated/discovery_stinger.wav | d020_pilot |
| `audio.sfx.ui_select` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.ui_confirm` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.ui_back` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.ui_error` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.ui_pause` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.footstep_common` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.player_landing` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.shield_armor_hit` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.player_damage` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.guard_start` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.guard_hit` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.parry_success` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.enemy_notice` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.enemy_tell` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.enemy_attack` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.enemy_damage` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.boss_intro_hit` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.boss_tell` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.boss_impact` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.boss_transition` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.boss_defeat` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.tool_equip_ready` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.tool_near_response` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.tool_cooldown_ready` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.pedestal_wake` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.mechanism_move` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.chest_open` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.relic_appear` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.pickup` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.system_save` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.system_load_continue` | SFX | needed | TBD | TBD | not_started |
| `audio.sfx.settings_apply` | SFX | needed | TBD | TBD | not_started |
| `audio.music.bgm_hub` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_region01` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_region02` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_region03` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_normal_combat` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_boss` | Music | needed | TBD | TBD | not_started |
| `audio.music.bgm_discovery_reward` | Music | needed | TBD | TBD | not_started |

## File Audit

- Repository audio files: 10
- Registered paths missing on disk: 0
- Unregistered audio files: 0
- Scene AudioSources: 2
- D-020 playable smoke audio wiring: passed

### Missing Registered Files

- None

### Unregistered Audio Files

- None

## License Coverage

- Registered cues: 49
- Cues with known license: 10
- Cues with Unity paths: 10
- License document mentions audio: true

## Known Gaps

- audio.sfx.ui_select, audio.sfx.ui_confirm, audio.sfx.ui_back, audio.sfx.ui_error, audio.sfx.ui_pause, audio.sfx.footstep_common, audio.sfx.player_landing, audio.sfx.shield_armor_hit, audio.sfx.player_damage, audio.sfx.guard_start, audio.sfx.guard_hit, audio.sfx.parry_success, audio.sfx.enemy_notice, audio.sfx.enemy_tell, audio.sfx.enemy_attack, audio.sfx.enemy_damage, audio.sfx.boss_intro_hit, audio.sfx.boss_tell, audio.sfx.boss_impact, audio.sfx.boss_transition, audio.sfx.boss_defeat, audio.sfx.tool_equip_ready, audio.sfx.tool_near_response, audio.sfx.tool_cooldown_ready, audio.sfx.pedestal_wake, audio.sfx.mechanism_move, audio.sfx.chest_open, audio.sfx.relic_appear, audio.sfx.pickup, audio.sfx.system_save, audio.sfx.system_load_continue, audio.sfx.settings_apply, audio.music.bgm_hub, audio.music.bgm_region01, audio.music.bgm_region02, audio.music.bgm_region03, audio.music.bgm_normal_combat, audio.music.bgm_boss, audio.music.bgm_discovery_reward remain registered but not implemented.
- Generated/prototype SFX are wired into D-020 gameplay evidence, but remain non-final release audio.
