...
{
    public sealed class ProductionCombatRewardClaimPrompt : MonoBehaviour
    {
        private const string TargetSceneName = "ProductionCombatSlice";
        private const float ControllerRefreshSeconds = 0.25f;
        public const float DefaultPromptRange = 2.35f;

        private static ProductionCombatRewardClaimPrompt instance;
        ...
        private static void EnsureInstance()
        {
            ...
            var gameObject = new GameObject(nameof(ProductionCombatRewardClaimPrompt));
            DontDestroyOnLoad(gameObject);
            instance = gameObject.AddComponent<ProductionCombatRewardClaimPrompt>();
        }
        ...
        private void OnGUI()
        {
            ...
            if (controller == null || controller.player == null || controller.rewardChest == null)
            {
                return;
            }
            ...
            if (!ShouldShowPrompt(
                    ...
                    controller.rewardChest.transform.position,
                    DefaultPromptRange))
            {
                return;
            }
            ...
            var accent = new Rect(rect.x + 12f, rect.y + 12f, 4f, rect.height - 24f);
        ...
        public static bool ShouldShowPrompt(
            ...
            float promptRange)
        {
            if (state != ProductionCombatRunState.Playing || !gateOpen || rewardClaimed)
            {
                return false;
            }
            ...
            playerPosition.y = 0f;
            rewardPosition.y = 0f;
            return Vector3.Distance(playerPosition, rewardPosition) <= Mathf.Max(0f, promptRange);
        }

        public static Rect BuildPromptRect(float screenWidth, float screenHeight)
        {
            var width = Mathf.Min(360f, Mathf.Max(0f, screenWidth - 48f));
            var height = 58f;
        ...
        private void EnsureStyles()
        {
            if (panelTexture == null)
            {
                ...
                panelTexture.SetPixel(0, 0, new Color(0.025f, 0.03f, 0.035f, 0.88f));
                panelTexture.Apply();
            }
            ...
            titleStyle.normal.textColor = new Color(0.95f, 0.9f, 0.78f, 1f);
            ...
            detailStyle.normal.textColor = new Color(0.72f, 0.76f, 0.72f, 1f);
        }
    }
}
