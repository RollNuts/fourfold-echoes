    ...
    public static class FourfoldProductValidator
    {
        ...
        public static void RunAll()
        {
            ...
            try
            {
                ...
                findings.Add(Finding.Info("d020.slice", "D-020 vertical slice generated and validated with one exploration tool, two tool nodes, shortcut route, two normal enemy types, elite guard, boss, basic-attack enemy defeat, enemy-hit failure, retry, title return, shared pause/settings UX, objective marker, progression rail, unbanked relic abandon confirmation, two distinct relic effects, return gate, failed-run reward loss, required SFX, two BGM clips, and full-loop reward banking."));
            }
            catch (Exception exception)
            {
            ...
            try
            {
                ...
                findings.Add(Finding.Info("hub.crossroads", "Hub Crossroads generated and validated as the playable hub with a D-020 region gate, mission briefing/start confirmation, returned-run summary/replay UX, objective marker, progress initialization, pause/settings UX, reset, and return-to-title persistence."));
            }
            catch (Exception exception)
            {
            ...
            try
            {
                ...
                findings.Add(Finding.Info("title.entry", "Title scene generated and validated with New Game, Continue, Settings volume persistence, Quit request, and Build Settings order Title -> HubCrossroads -> D020VerticalSlice."));
            }
            catch (Exception exception)
            {
