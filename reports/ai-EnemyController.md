...
Batchmode editor verification:

- `FourfoldEchoes.Editor.FourfoldEnemyAiVerificationSceneBuilder.RunStateTransitionValidation`
  validates combat-loop/death, leash return, line-of-sight wall blocking, and fallback obstacle blocking without leaving cross-case colliders in the physics scene.
- `FourfoldEchoes.Editor.FourfoldEnemyAiVerificationSceneBuilder.BuildAndValidate`
  creates the verification scene and asserts the required target, wall, enemy controllers, definitions, sensors, motors, attacks, and damageables.
